...
#  ✓ fabio  hostname ⏻ 94% ⏳ ~/bin master  ✔ 
#  |   |        |      |     |    |     |- Git status
#  |   |        |      |     |    |- PWD
#  |   |        |      |     |- sudo status
#  |   |________|      |- battery indicator and percentage
#  |        |- User and hostname
#  |- $?
...
_git_branch ()
{
  ...
  if [ "$gitbranch" ] ; then
    ...
    git_status=$(timeout --signal=SIGINT 3 git status --porcelain 2> /dev/null)
    if [ "$git_status" ] ; then
      modified="$(echo $git_status | grep -v '^?' | wc -l)"
    ...
    ahead_behind=$(git branch -v | grep '*' 2> /dev/null | grep 'behind\|ahead' | awk '{print $4}')
